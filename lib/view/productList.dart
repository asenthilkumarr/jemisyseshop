...
class productlistParam{
  List<Product> productdt;
  String title;
  String fsource;
  String filterType;
  GlobalKey<FormState> masterScreenFormKey;
}
...
class ProductListPage2 extends StatefulWidget{
  ...
  @override
  _productListPage2 createState() => _productListPage2();
}
class _productListPage2 extends State<ProductListPage2> {
...
//                          color: source == "groupName" ? buttonShadowColor : Colors.white,
  ...
  Widget homeWidget(double screenHeight, double screenWidth) {
    ...
    //final routes=ModalRoute.of(context).settings.arguments as productlistParam;
    final routes=new productlistParam();
                    ...
                    delegate: SliverChildListDelegate(
                      [
                        for(var i in fproductdt)
                          ProductGridWidget(item: i, masterScreenFormKey: routes.masterScreenFormKey,),
                      ],
                    ),
                  ),
                  SliverToBoxAdapter(
...
//     productdt = new List<Product>();
//     fproductdt = new List<Product>();
//     productdt = dt;
//     fproductdt = dt;
//     totItem = dt.length;
//     print(productdt.length);
//     setState(() {
//
//     });
    return dt;
  ...
  void loadDefault() async{
    // List<Product> args = ModalRoute.of(context).settings.arguments;
    final routes=ModalRoute.of(context).settings.arguments as productlistParam;
    // final routes= productlistParam();
    if(kIsWeb)
      isWeb = true;
    if(routes.fsource == "MENU"){
      if(routes.filterType == "WATCHONLY"){
        productdt = await getProductDetail("WATCHES", routes.filterType);
      }
      else {
        await getGroup();
        productdt = await getProductDetail(_selgroup, routes.filterType);
      }
      fproductdt = productdt;
    }
    else{
      productdt = routes.productdt;
      fproductdt = routes.productdt;
...
//                        style: TextStyle(color: Colors.white),
  ...
  Widget build(BuildContext context) {
    ...
    if(rname.length>1)
      category = (routename.split("?")[1]).split("=")[1];
      print(category);
    // return MaterialApp(
    //   title: 'Product List',
...
//        drawer: filterWidget2(1,100),
          body: SafeArea(
            child: productdt != null && productdt.length >0? Container(
                color: Colors.white,
                child: Stack(
                    children: [
                      Row(
